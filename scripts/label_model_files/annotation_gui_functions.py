...
class AnnotationTool:
    def __init__(self, root, default_label=None, version='version1.2'):
        self.root = root
    ...
    def save_annotations(self):
        label_dir = Path.cwd() / 'model_training/data/labels'
        ...
        for image_path_str in self.images:
            ...
            # Change here: Use the full file name with .txt extension instead of just the stem
            file_name = f"{current_image_path.stem}.txt"
            # Ensure we replace the original extension with .txt
            # file_name = file_name.rsplit('.', 1)[0] + '.txt'
            
            # print(file_name)  # Just to verify what's being saved
