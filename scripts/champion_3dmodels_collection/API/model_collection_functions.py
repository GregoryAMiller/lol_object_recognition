...
def update_yaml_file_if_needed(yaml_file_path, last_run_file_path):
    """
    Check if the YAML file needs to be updated based on the last run time.
    If it hasn't been updated in three weeks, update it.
    """
    try:
        with open(last_run_file_path, 'r') as file:
            last_run_time_str = file.readline().strip().replace("Last run time: ", "")
            last_run_time = datetime.datetime.strptime(last_run_time_str, "%Y-%m-%d %H:%M:%S")
    except (FileNotFoundError, ValueError):
        last_run_time = datetime.datetime.min  # If file not found or error in reading, set to min to ensure update

    if datetime.datetime.now() - last_run_time > datetime.timedelta(weeks=3):
        current_champions_data = model_collection_functions.get_champions_data()
        existing_data = load_yaml_file(yaml_file_path)
        if current_champions_data != existing_data:
            champions_data_with_skins = model_collection_functions.add_skin_data(current_champions_data)
            model_collection_functions.save_champion_skin_mapping(champions_data_with_skins, yaml_file_path)
            model_collection_functions.record_last_run_time(last_run_file_path)
            print("YAML file and last run time updated with the latest champions data.")
        else:
            print("No updates required for the YAML file. Updating last run time only.")
            model_collection_functions.record_last_run_time(last_run_file_path)
    else:
        print("YAML file is up-to-date within the last three weeks.")