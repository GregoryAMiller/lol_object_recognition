...
model = YOLO(f'{Path.cwd()}/runs/detect/train6/weights/best.pt')  # Path to the trained weights
...
source_dir = Path(f'{Path.cwd().parent}/Ahri_Academia___Previsualizaci_n___League_of_Legends_frames')  # Update with your source path
...
for image_path in source_dir.glob('*.jpg'):  # Adjust for different image formats as necessary
    ...
    model.predict(str(image_path), save=True, conf=0.25) # , imgsz=320
